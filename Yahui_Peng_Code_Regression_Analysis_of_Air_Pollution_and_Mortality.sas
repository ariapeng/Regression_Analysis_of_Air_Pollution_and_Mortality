...
/* import database after deleting point 7, 17*/
FILENAME REFFILE '/home/u49400069/Regression Analysis/Final_Project/Delete x4 outliers.xls';
...
/* import database after deleting point 4,6,56,57*/
FILENAME REFFILE '/home/u49400069/Regression Analysis/Final_Project/Final_Project_delete_4influentials.csv';
PROC IMPORT DATAFILE=REFFILE
	DBMS=CSV
	OUT=pollution;
	GETNAMES=YES;
...
PROC CONTENTS DATA=pollution; RUN;
...
/* weighted ls est */
data pollution_no_outlier;
	set pollution end=last;
	if not last then
		output;
run;
data pollution_no_outlier2;
	set pollution_no_outlier end=last;
	if not last then
		output;
run;
* step0: initial step ;
